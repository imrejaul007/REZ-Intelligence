/**
 * REZ Moment Ads - RABTUL Integration
 */
...
export async function rewardUser(userId: string, amount: number) {
  try { await axios.post(`${WALLET}/api/wallet/add`, { userId, amount, reason: 'moment_ad_reward' }, { headers: { 'Content-Type': 'application/json', 'X-Internal-Token': TOKEN } }); return { success: true }; }
  catch { return { success: false }; }
}
export const momentAdsRABTUL = { verifyToken, rewardUser };
export default momentAdsRABTUL;
