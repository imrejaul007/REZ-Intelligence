/**
 * REZ Aggregator Hub - RABTUL Integration
 */
...
export async function publishEvent(type: string, data: Record<string, any>) {
  try { await axios.post(`${EVENT}/api/events/publish`, { type: `aggregator.${type}`, source: 'rez-aggregator-hub', data }, { headers: { 'X-Internal-Token': TOKEN } }); return { success: true }; }
  catch { return { success: false }; }
}
export const aggregatorHubRABTUL = { verifyToken, publishEvent };
export default aggregatorHubRABTUL;
