...
class PatternMatcher {
    ...
    async checkCardTesting(context) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.CARD_TESTING];
        ...
        if (score >= 40) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.CARD_TESTING,
                    pattern,
                    score,
                    confidence,
                    evidence,
    ...
    async checkImpossibleTravel(context, riskProfile) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.IMPOSSIBLE_TRAVEL];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        ...
        if (score >= 60) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.IMPOSSIBLE_TRAVEL,
                    pattern,
                    score,
                    confidence: 80,
                    evidence,
    ...
    async checkBillingShippingMismatch(context) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.BILLING_SHIPPING_MISMATCH];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        ...
        if (score >= 40) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.BILLING_SHIPPING_MISMATCH,
                    pattern,
                    score,
    ...
    async checkNewDeviceAnomaly(context, riskProfile) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.NEW_DEVICE_ANOMALY];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        ...
        if (score >= 40) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.NEW_DEVICE_ANOMALY,
                    pattern,
                    score,
    ...
    async checkUnusualAmount(context, riskProfile) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.UNUSUAL_AMOUNT];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        ...
        if (score >= 40) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.UNUSUAL_AMOUNT,
                    pattern,
                    score,
    ...
    async checkBotBehavior(context) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.BOT_BEHAVIOR];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        // Very short session
        ...
        if (context.navigationPattern && context.navigationPattern.length > 0) {
            const hasNoProductPages = !context.navigationPattern.some(p => p.includes('product') || p.includes('browse') || p.includes('search'));
        ...
        if (score >= 50) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.BOT_BEHAVIOR,
                    pattern,
                    score,
    ...
    async checkGeoAnomaly(context, riskProfile) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.GEO_ANOMALY];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        ...
        if (riskProfile && riskProfile.usualLocations.length > 0) {
            const isKnownCountry = riskProfile.usualLocations.some(loc => loc.country === context.billingCountry);
            if (!isKnownCountry) {
        ...
        if (score >= 35) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.GEO_ANOMALY,
                    pattern,
                    score,
    ...
    async checkSessionAnomaly(context) {
        const pattern = patterns_1.FRAUD_PATTERNS[patterns_1.FraudPatternType.SESSION_ANOMALY];
        const evidence = {};
        const riskFactors = [];
        let score = 0;
        // Very short session
        ...
        if (score >= 30) {
            return {
                isMatch: true,
                match: {
                    patternType: patterns_1.FraudPatternType.SESSION_ANOMALY,
                    pattern,
                    score,
                    confidence: 60,
                    evidence,
