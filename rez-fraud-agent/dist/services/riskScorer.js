...
class RiskScorer {
    ...
    async calculateScore(input) {
        ...
        const totalScore = Math.min(100, Math.round(breakdown.components.patternScore * this.WEIGHTS.patternScore +
    ...
    async calculateBreakdown(input) {
        const factors = [];
    ...
    getRiskLevel(score) {
        if (score >= 90)
            return RiskProfile_1.RiskLevel.CRITICAL;
        if (score >= 75)
            return RiskProfile_1.RiskLevel.HIGH;
        if (score >= 50)
            return RiskProfile_1.RiskLevel.ELEVATED;
        if (score >= 25)
            return RiskProfile_1.RiskLevel.NORMAL;
        return RiskProfile_1.RiskLevel.TRUSTED;
    }
    analyzeBehavioralFactors(context) {
        const factors = [];
        let score = 0;
    ...
    analyzeHistoricalFactors(profile, context) {
        const factors = [];
        let score = 0;
        ...
        if (context.billingCountry && profile.usualLocations.length > 0) {
            const isUsualLocation = profile.usualLocations.some(loc => loc.country === context.billingCountry);
            if (!isUsualLocation) {
                score += 25;
