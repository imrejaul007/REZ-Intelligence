...
export class TransactionMonitor {
  ...
  private readonly KEYS = {
    failedAttempts: (userId: string) => `fraud:failed:${userId}`,
    transactionAmounts: (userId: string) => `fraud:amounts:${userId}`,
    transactionCountries: (userId: string) => `fraud:countries:${userId}`,
    merchantCategories: (userId: string) => `fraud:mcc:${userId}`,
  };
  ...
  async setRedisClient(redis: RedisClientType): Promise<void> {
    this.redis = redis;
  }

  async monitor(context: TransactionContext): Promise<TransactionMonitorResult> {
  ...
  private async checkFailedAttempts(context: TransactionContext): Promise<TransactionAnomaly[]> {
    if (!this.redis || !context.userId) return [];
    ...
    try {
      ...
      logger.error('Failed to check failed attempts', { error, userId: context.userId });
    }

    return anomalies;
  }

  private async checkAmountPatterns(context: TransactionContext): Promise<TransactionAnomaly[]> {
    if (!this.redis || !context.userId) return [];
    ...
    try {
      ...
      const amounts = recentAmounts.map((a: string) => parseFloat(a)).filter((a: number) => !isNaN(a));

      if (amounts.length >= 3) {
        const average = amounts.reduce((sum: number, a: number) => sum + a, 0) / amounts.length;
      ...
      logger.error('Failed to check amount patterns', { error, userId: context.userId });
    }

    return anomalies;
  }

  private async checkLocationPatterns(context: TransactionContext): Promise<TransactionAnomaly[]> {
    if (!this.redis || !context.userId) return [];
    ...
    try {
      ...
      const countries = recentCountries.filter((c: string) => c !== '');
      ...
      logger.error('Failed to check location patterns', { error, userId: context.userId });
    }

    return anomalies;
  }

  private async checkMerchantPatterns(context: TransactionContext): Promise<TransactionAnomaly[]> {
    if (!this.redis || !context.userId) return [];

    const anomalies: TransactionAnomaly[] = [];

    if (!context.merchantCategory) return anomalies;
    ...
    try {
      ...
      const mccs = recentMCCs.filter((m: string) => m !== '');
      ...
      if (highRiskMCCs.includes(context.merchantCategory)) {
        ...
        const previousUsage = mccs.filter((m: string) => m === context.merchantCategory).length;
      ...
      logger.error('Failed to check merchant patterns', { error, userId: context.userId });
    }
  ...
  async recordFailedAttempt(userId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to record failed attempt', { error, userId });
    }
  }
  ...
  async clearFailedAttempts(userId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to clear failed attempts', { error, userId });
    }
  }
}
