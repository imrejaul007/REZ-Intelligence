...
export class VelocityCheck {
  ...
  async check(context: TransactionContext): Promise<VelocityCheckResult> {
    const baseResult: VelocityCheckResult = {
      isViolation: false,
      score: 0,
      evidence: {},
      riskFactors: [],
  ...
  ): Promise<{ count: number; isViolation: boolean; score: number }> {
    if (!this.redis) {
  ...
  private async recordTransaction(userId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to record velocity', { error, userId });
    }
  }
  ...
  async getVelocityStats(userId: string): Promise<{
    ...
    return {
      ...
      recentTransactions: transactions.map((t: string) => parseInt(t, 10)),
    };
  }

  async resetVelocity(userId: string): Promise<void> {
    if (!this.redis) return;
    ...
    try {
      ...
      logger.error('Failed to reset velocity', { error, userId });
    }
  }

  async checkIPVelocity(ipAddress: string): Promise<VelocityCheckResult> {
    if (!this.redis) {
      return {
        isViolation: false,
        score: 0,
