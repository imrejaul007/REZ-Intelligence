...
export function getPatternScore(
  ...
  const pattern = FRAUD_PATTERNS[patternType];
  if (!pattern) return 0;

  let score = pattern.baseScore;

  // Adjust score based on context factors
  const severity = context.severity as number;
  if (severity) {
...
export function getPatternsByCategory(category: string): FraudPattern[] {
  return Object.values(FRAUD_PATTERNS).filter((pattern) =>
    pattern.indicators.some((indicator) =>
      indicator.toLowerCase().includes(category.toLowerCase())
    )
  );
}
