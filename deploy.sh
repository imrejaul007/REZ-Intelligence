#!/bin/bash
# =============================================================================
# REZ-Intelligence Deployment Script
# Deploys all AI/ML and intelligence services to Render
# =============================================================================
...
LOG_FILE="${SCRIPT_DIR}/logs/deploy-rez-intelligence_${TIMESTAMP}.log"
...
# Nested services
NESTED_SERVICES=(
    "rez-ai-platform/services/targeting-engine"
    "rez-ai-platform/services/personalization-engine"
    "rez-ai-platform/services/observability"
    "rez-ai-platform/services/recommendation-engine"
    "rez-ai-platform/services/support-copilot"
    "rez-ai-platform/services/push-service"
)

ALL_SERVICES=("${AI_SERVICES[@]}" "${INTELLIGENCE_SERVICES[@]}" "${INFRA_SERVICES[@]}" "${ML_SERVICES[@]}" "${NESTED_SERVICES[@]}")
...
check_prerequisites() {
    log_header "REZ-INTELLIGENCE PRE-DEPLOYMENT CHECKS"
...
main() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                  REZ-INTELLIGENCE DEPLOYMENT                  ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    log_header "DEPLOYING ${total_services} AI/INTELLIGENCE SERVICES"
    ...
    # Print summary
    log_header "DEPLOYMENT SUMMARY - REZ-INTELLIGENCE"
    ...
    if [ $FAILED_COUNT -eq 0 ]; then
        log_success "All REZ-Intelligence services deployed successfully!"
    else
