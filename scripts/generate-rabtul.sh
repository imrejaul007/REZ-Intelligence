#!/bin/bash
# Script to generate rabtul.ts for all services missing it

cat > template_rabtul.ts << 'EOF'
/**
 * SERVICE_NAME - RABTUL Integration
 */
...
export async function publishEvent(type: string, data: Record<string, any>): Promise<{ success: boolean }> {
  try {
    await axios.post(`${EVENT_BUS_URL}/api/events/publish`, { type, source: 'SERVICE_NAME', data }, {
      headers: { 'X-Internal-Token': INTERNAL_TOKEN },
    });
    return { success: true };
  } catch { return { success: false }; }
...
export const SERVICE_NAME_CAMELCaseRABTUL = { verifyToken, addCoins, notifyUser, publishEvent };
export default SERVICE_NAME_CAMELCaseRABTUL;
EOF
