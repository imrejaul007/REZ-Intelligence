/**
 * REZ Realtime Service - RABTUL Integration
 */
...
export async function publishEvent(type: string, data: Record<string, unknown>) {
  try { await axios.post(`${EVENT}/api/events/publish`, { type, source: 'REZ-realtime-service', data }, { headers: { 'X-Internal-Token': TOKEN } }); return { success: true }; }
  catch { return { success: false }; }
}
export const realtimeServiceRABTUL = { verifyToken, publishEvent };
export default realtimeServiceRABTUL;
