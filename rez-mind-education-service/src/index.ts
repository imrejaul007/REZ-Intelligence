...
app.get('/health', (req: Request, res: Response) => {
  res.status(200).json({
    status: 'healthy',
    service: 'rez-mind-education-service',
...
app.use('/api/performance', internalAuth, readLimiter, performanceRoutes);
app.use('/api/attendance', internalAuth, readLimiter, attendanceRoutes);
app.use('/api/enrollment', internalAuth, readLimiter, enrollmentRoutes);
...
async function startServer(): Promise<void> {
  try {
    ...
    server = app.listen(port, () => {
      logger.info(`ReZ Mind Education Service started successfully`, {
        port,
        env: config.nodeEnv,
      ...
      logger.info('Available endpoints:', {
        ...
        performance: {
          list: `GET http://localhost:${port}/api/performance/:institutionId`,
          student: `GET http://localhost:${port}/api/performance/:institutionId/student/:studentId`,
          predict: `POST http://localhost:${port}/api/performance/:institutionId/predict`,
        },
        attendance: {
          list: `GET http://localhost:${port}/api/attendance/:institutionId`,
          student: `GET http://localhost:${port}/api/attendance/:institutionId/student/:studentId`,
          analyze: `POST http://localhost:${port}/api/attendance/:institutionId/analyze`,
        },
        enrollment: {
          courses: `GET http://localhost:${port}/api/enrollment/:institutionId/courses`,
          recommend: `POST http://localhost:${port}/api/enrollment/:institutionId/recommend`,
          dropoutRisk: `GET http://localhost:${port}/api/enrollment/:institutionId/dropout-risk`,
        },
      });
    });
