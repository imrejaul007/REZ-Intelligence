...
const envSchema = z.object({
  PORT: z.coerce.number().int().min(1).max(65535).default(4058),
  NODE_ENV: z.enum(['development', 'production', 'test']).default('development'),
  MONGODB_URI: z.string().url().default('mongodb://localhost:27017/rez_mind_education'),
  ...
  RABTUL_PLATFORM_URL: z.string().url().optional(),
});
...
export const config = {
  ...
  rabtulPlatformUrl: envConfig.RABTUL_PLATFORM_URL || 'https://rabtul.rez.com',
};
