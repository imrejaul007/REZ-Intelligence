...
export async function publishEvent(type: string, data: Record<string, unknown>) {
  try { await axios.post(`${EVENT}/api/events/publish`, { type: `commerce.${type}`, source: 'REZ-unified-commerce-graph', data }, { headers: { 'X-Internal-Token': TOKEN } }); return { success: true }; }
