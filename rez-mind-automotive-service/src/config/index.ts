...
const envSchema = z.object({
  ...
  PORT: z.coerce.number().int().min(1024).max(65535).default(4061),
  ...
  RATE_LIMIT_MAX_AI: z.coerce.number().int().positive().default(30),
...
export const config = {
  ...
  ai: {
    ...
    maxTokens: envConfig.AI_MAX_TOKENS,
  ...
  session: {
    ttlDays: envConfig.SESSION_TTL_DAYS,
  },
