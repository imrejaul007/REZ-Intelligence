...
export const intentOperations = {
  async predict(userId: string, context: Record<string, unknown> = {}) {
    try {
      const res = await internalRequest<unknown>(`${INTENT_SERVICE_URL}/api/intent/predict`, {
        method: 'POST',
  ...
  async captureSignal(signal: unknown) {
    try {
...
export const predictiveOperations = {
  async predictChurn(userId: string) {
    try {
      const res = await internalRequest<unknown>(`${PREDICTIVE_SERVICE_URL}/predict/${userId}/churn`);
      return res;
    } catch {
      return null;
    }
  ...
  async predictLTV(userId: string) {
    try {
      const res = await internalRequest<unknown>(`${PREDICTIVE_SERVICE_URL}/predict/${userId}/ltv`);
      return res;
    } catch {
      return null;
    }
  ...
  async predictRevisit(userId: string) {
    try {
      const res = await internalRequest<unknown>(`${PREDICTIVE_SERVICE_URL}/predict/${userId}/revisit`);
      return res;
    } catch {
      return null;
    }
