...
export const intentOperations = {
  async predict(userId: string, context: IntentContext = {}): Promise<unknown | null> {
    try {
  ...
  async captureSignal(signal: SignalData): Promise<boolean> {
    try {
...
export const signalOperations = {
  async record(signal: SignalData): Promise<boolean> {
    try {
