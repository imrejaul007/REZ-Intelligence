/**
 * REZ Commerce Signal Connector - RABTUL Integration
 */
...
export async function publishEvent(type: string, data: Record<string, unknown>) {
  try { await axios.post(`${EVENT}/api/events/publish`, { type: `commerce.${type}`, source: 'REZ-commerce-signal-connector', data }, { headers: { 'X-Internal-Token': TOKEN } }); return { success: true }; }
  catch { return { success: false }; }
}
export const commerceSignalConnectorRABTUL = { verifyToken, addCoins, publishEvent };
export default commerceSignalConnectorRABTUL;
