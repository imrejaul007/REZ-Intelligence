...
export const authOperations = {
  async verify(token: string): Promise<User | null> {
    try {
      const res = await internalRequest<{ success: boolean; user?: User }>(`${AUTH_URL}/api/auth/verify`, {
        method: 'POST',
...
export const walletOperations = {
  ...
  async getTransactions(userId: string, limit = 20): Promise<Transaction[]> {
    try {
      const res = await internalRequest<{ transactions: Transaction[] }>(`${WALLET_URL}/api/wallet/${userId}/transactions?limit=${limit}`);
...
export const eventBusOperations = {
  async publish(type: string, category: string, data: Record<string, unknown>, context: EventBusContext = {}): Promise<boolean> {
    try {
