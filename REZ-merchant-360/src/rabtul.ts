/**
 * REZ Merchant 360 - RABTUL Integration
 */
...
export async function addEarnings(merchantId: string, amount: number, reason: string) {
  try { await axios.post(`${WALLET}/api/wallet/add`, { userId: merchantId, amount, reason }, { headers: { 'Content-Type': 'application/json', 'X-Internal-Token': TOKEN } }); return { success: true }; }
...
export const merchant360RABTUL = { verifyToken, getWallet, addEarnings, notifyMerchant };
export default merchant360RABTUL;
