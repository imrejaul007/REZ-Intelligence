...
class RecommendationsService {
     ...
     */
    async getPersonalizedRecommendations(userId, menuItems, context, limit = 5) {
        ...
        for (const item of menuItems) {
            if (!item.available)
                continue;
            const { isCompatible } = await this.dietaryService.checkDishCompatibility(userId, item.description, item.allergens, item.dietaryTags);
            if (!isCompatible && !context.cuisinePreference)
                continue;
            ...
            if (userProfile?.preferences.preferredCuisines) {
                const historyMatch = userProfile.preferences.preferredCuisines.some(pref => item.cuisine?.toLowerCase().includes(pref.toLowerCase()));
                if (historyMatch) {
                    score += 15;
            ...
            if (userProfile?.restrictions) {
                const matchingTags = item.dietaryTags.filter(tag => userProfile.restrictions.includes(tag));
     ...
     */
    async buildMealPlan(userId, menuItems, context) {
        const recommendations = await this.getPersonalizedRecommendations(userId, menuItems, context, 10);
        const mealItems = [];
        let totalPrice = 0;
        let totalCalories = 0;
        ...
        if (starters.length > 0 && context.occasion !== 'quick') {
            ...
            if (starters[0].item.calories)
                totalCalories += starters[0].item.calories;
        ...
        if (mains.length > 0) {
            ...
            if (mains[0].item.calories)
                totalCalories += mains[0].item.calories;
        ...
        if (context.occasion === 'celebration' || context.occasion === 'date') {
            ...
            if (desserts.length > 0) {
                ...
                if (desserts[0].item.calories)
                    totalCalories += desserts[0].item.calories;
     ...
     */
    async getComplementaryItems(userId, selectedItem, menuItems) {
        const availableItems = menuItems.filter(item => item.id !== selectedItem.id && item.available);
        const recommendations = [];
    ...
    getOccasionScore(item, occasion) {
    ...
    getTimeOfDayScore(item, timeOfDay) {
    ...
    getCuisineDefaultPairing(cuisine) {
        const cuisineLower = cuisine.toLowerCase();
        if (cuisineLower.includes('italian'))
            return 'Italian Red Wine';
        if (cuisineLower.includes('japanese'))
            return 'Sake or Japanese Beer';
        if (cuisineLower.includes('mexican'))
            return 'Mexican Lager or Tequila';
        if (cuisineLower.includes('indian'))
            return 'India Pale Ale';
        if (cuisineLower.includes('french'))
            return 'French Wine';
        if (cuisineLower.includes('thai') || cuisineLower.includes('chinese'))
            return 'Rice Wine or Tea';
        return null;
    }
    getDefaultWinePairing(item) {
