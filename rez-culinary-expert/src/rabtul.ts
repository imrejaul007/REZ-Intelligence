/**
 * Culinary Expert - RABTUL Integration
 */
...
export async function createPayment(userId: string, amount: number, description: string) {
  try {
    const res = await axios.post(`${PAYMENT_URL}/api/payments/create`, {
      userId, amount, description, metadata: { source: 'culinary-expert' }
    }, {
...
export const culinaryRABTUL = { verifyToken, createPayment, addReward };
