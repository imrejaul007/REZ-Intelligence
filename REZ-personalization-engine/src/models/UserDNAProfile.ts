/**
 * UserDNAProfile Model - TypeScript implementation
...
const preferenceVectorSchema = new Schema<IPreferenceVector>({
  dimension: { type: String, required: true },
  value: { type: Number, default: 0, min: -1, max: 1 },
  updatedAt: { type: Date, default: Date.now },
}, { _id: false });

const contentAffinitySchema = new Schema<IContentAffinity>({
...
const notificationTimingPreferenceSchema = new Schema<INotificationTimingPreference>({
...
const userDNAProfileSchema = new Schema<IUserDNAProfileDocument>({
  userId: { type: String, required: true, unique: true, index: true },

  // Core Personalization Data
  behavioralPatterns: [{ type: behavioralPatternSchema }],
  preferenceVector: [{ type: preferenceVectorSchema }],
  contentAffinityScores: [{ type: contentAffinitySchema }],

  // Communication & Timing
  communicationStyle: {
    type: String,
    enum: ['formal', 'casual', 'friendly', 'professional', 'mixed'] as CommunicationStyle[],
    default: 'mixed',
  },
  notificationTimingPreference: {
    type: notificationTimingPreferenceSchema,
    default: () => ({}),
  },
  ...
  priceSensitivityTier: {
    type: String,
    enum: ['budget', 'moderate', 'premium', 'luxury', 'insensitive'] as PriceSensitivityTier[],
    default: 'moderate',
  },
  ...
  activityFrequency: {
    type: String,
    enum: ['daily', 'weekly', 'monthly', 'occasional'] as ActivityFrequency[],
    default: 'weekly',
  },
  ...
  source: {
    type: String,
    enum: ['explicit', 'implicit', 'hybrid'] as ProfileSource[],
    default: 'implicit',
  },
}, {
  timestamps: true,
...
userDNAProfileSchema.index({
  'contentAffinityScores.category': 1,
  'contentAffinityScores.score': -1,
});
...
userDNAProfileSchema.methods.updatePreference = function (
  this: IUserDNAProfileDocument,
  dimension: string,
  value: number
): this {
...
userDNAProfileSchema.methods.updateAffinity = function (
  this: IUserDNAProfileDocument,
  contentType: string,
  category: string,
  score: number,
  interaction: string = 'view'
): this {
  ...
  const alpha = 0.2;
  const newScore = existing ? existing.score + alpha * (score - existing.score) : score;
...
userDNAProfileSchema.methods.addBehavioralPattern = function (
  this: IUserDNAProfileDocument,
  type: string,
  pattern: Partial<IBehavioralPattern>
): this {
...
userDNAProfileSchema.methods.calculateCompleteness = function (
  ...
  checks.forEach((check) => {
    if ((check.field as number) >= check.min) {
      score += check.weight;
    } else {
      score += ((check.field as number) / check.min) * check.weight;
    }
  });
...
userDNAProfileSchema.statics.findOrCreate = async function (
  this: Model<IUserDNAProfileDocument>,
  userId: string
): Promise<IUserDNAProfileDocument> {
...
userDNAProfileSchema.statics.getSimilarUsers = async function (
  this: Model<IUserDNAProfileDocument>,
  userId: string,
  limit: number = 10
): Promise<Array<{ userId: string; similarityScore: number; engagementScore: number }>> {
  ...
  const topBrands = user.brandPreferences.slice(0, 5).map((b) => b.brandId);
  ...
  const similarUsers = await this.aggregate([
    ...
    },
    {
      $addFields: {
        categoryMatch: {
          $size: {
            $setIntersection: [topCategories, '$contentAffinityScores.category'],
          },
        },
        brandMatch: {
          $size: {
            $setIntersection: [topBrands, '$brandPreferences.brandId'],
          },
        },
      },
    },
...
export const UserDNAProfile = mongoose.model<IUserDNAProfileDocument, IUserDNAProfileModel>(
  'UserDNAProfile',
  userDNAProfileSchema
);

export default UserDNAProfile;
