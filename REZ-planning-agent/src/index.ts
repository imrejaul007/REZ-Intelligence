...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/planning-agent';
...
app.get('/', (_req, res) => {
  res.json({
    service: 'REZ Planning Agent',
    version: '1.0.0',
    description: 'AI-powered strategic planning and task decomposition',
    endpoints: {
      health: 'GET /health',
      createPlan: 'POST /api/planning/plans',
      getPlan: 'GET /api/planning/plans/:id',
      executePlan: 'POST /api/planning/plans/:id/execute',
      getProgress: 'GET /api/planning/plans/:id/progress',
      getNextTask: 'GET /api/planning/plans/:id/next',
      updateTask: 'POST /api/planning/plans/:id/tasks/update',
      getExecution: 'GET /api/planning/plans/:id/execution',
    },
  });
});
...
async function startServer(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`REZ Planning Agent running on port ${PORT}`);
    });
