...
export function createWorkoutPlan(userProfile: UserProfile): WorkoutPlan {
  const planId = uuidv4();
...
function generatePlanName(profile: UserProfile): string {
  const prefixes: Record<FitnessLevel, string> = {
...
function generateExercisesForProfile(profile: UserProfile): PlannedExercise[] {
...
function getRepsForLevel(level: FitnessLevel, type: 'strength' | 'cardio'): string {
  switch (level) {
    case FitnessLevel.BEGINNER:
...
function getPlankTimeForLevel(level: FitnessLevel): string {
  switch (level) {
    case FitnessLevel.BEGINNER:
...
function generateWorkoutTips(profile: UserProfile): string[] {
  const tips: string[] = [
...
export function getExercisesByType(type: WorkoutType): typeof EXERCISE_DATABASE {
  return EXERCISE_DATABASE.filter(ex => ex.workoutType === type);
}

export function getExercisesByMuscle(muscleGroup: string): typeof EXERCISE_DATABASE {
...
export function calculateProgress(profile: UserProfile): ProgressUpdate {
...
export function getFitnessTerm(term: string): { term: string; definition: string } | null {
