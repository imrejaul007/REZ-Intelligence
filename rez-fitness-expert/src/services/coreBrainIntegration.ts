...
export class CoreBrainClient {
  ...
  buildFitnessContext(context: {
    ...
    const fitnessAchievements: string[] = [];
    const preferredWorkoutTypes: string[] = [];

    // Extract workout history from memories
    for (const memory of context.workoutHistory) {
      if (memory.metadata?.workout) {
        recentWorkouts.push(memory.metadata.workout as WorkoutRecord);
      }
    ...
    const recentCount = recentWorkouts.filter(
      (w) => new Date(w.date) > weekAgo
    ).length;

    let engagementLevel: 'low' | 'medium' | 'high' = 'low';
    if (recentCount >= 4) engagementLevel = 'high';
    else if (recentCount >= 2) engagementLevel = 'medium';
