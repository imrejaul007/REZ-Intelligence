/**
 * REZ Finance Expert Agent
 * AI-powered finance expert for financial planning, investment advice, and expense tracking
 */
...
const configSchema = z.object({
  port: z.string().optional().transform(val => val ? parseInt(val, 10) : 3014),
...
function loadConfig(): Config & { serviceName: string; version: string } {
  ...
  return {
    ...result.data,
    serviceName: 'rez-finance-expert',
    version: '1.0.0'
  };
}
...
app.get('/', (_req: Request, res: Response) => {
  res.json({
    service: config.serviceName,
    version: config.version,
    description: 'AI-powered finance expert for financial planning, investment advice, and expense tracking',
    endpoints: {
      health: 'GET /health',
      chat: 'POST /api/v1/finance/chat',
      analyze: 'POST /api/v1/finance/analyze',
      plan: 'POST /api/v1/finance/plan',
      investments: 'GET /api/v1/finance/investments',
      expenses: 'POST /api/v1/finance/expenses'
    }
  });
});
...
function startServer(): void {
  try {
    server = app.listen(config.port, () => {
      logger.info(`REZ Finance Expert started on port ${config.port}`);
      logger.info('Endpoints: chat, analyze, plan, investments, expenses');
    });
